...
def ensure_compatible_schema(connection) -> None:
    with connection.cursor() as cursor:
        ...
        for column_sql in [
            ...
            cursor.execute(f"ALTER TABLE exam.questions ADD COLUMN IF NOT EXISTS {column_sql}")

        cursor.execute("ALTER TABLE ingestion.embeddings ADD COLUMN IF NOT EXISTS source_kind TEXT")

        cursor.execute("ALTER TABLE exam.textbook_chunks ADD COLUMN IF NOT EXISTS chunk_text TEXT NOT NULL DEFAULT ''")

        cursor.execute(
            """
            UPDATE ingestion.embeddings e
            SET source_kind = s.source_kind
            FROM ingestion.pdf_chunks pc
            JOIN ingestion.sources s ON s.source_id = pc.source_id
            WHERE e.chunk_id = pc.chunk_id
              AND e.source_kind IS DISTINCT FROM s.source_kind
            """
        )
